...
public class StabilityUtils {
    ...
    public static DoubleMatrix2D StabilitySearchPar(final DataSet data, final DataGraphSearch gs, final int[][] subs, int r)
    {
        final int numVars = data.getNumColumns();
        final DoubleMatrix2D thetaMat = DoubleFactory2D.dense.make(numVars, numVars, 0.0).copy();
    ...
    public static DoubleMatrix2D StabilitySearchPar(final DataSet data, final DataGraphSearch gs, int r)
    {
        final int numVars = data.getNumColumns() - r;
        final DoubleMatrix2D thetaMat = DoubleFactory2D.dense.make(numVars, numVars, 0.0);
...
//            samp = subSampleNoReplacement(data.getNumRows(), b, N);
    ...
    //returns an adjacency matrix containing the edgewise instability as defined in Liu et al with survival
    public static DoubleMatrix2D StabilitySearchPar(final DataSet data, final DataGraphSearch gs, int N, int b, int r){
        ...
        class StabilityAction extends RecursiveAction{
            ...
            protected void compute(){
                if (to - from <= chunk) {
                    for (int s = from; s < to; s++) {
                        System.out.println(Thread.currentThread().getName() + ": Running sample: " + s);
                        DataSet dataSubSamp = data.subsetRows(samps[s]).copy();
                        DataGraphSearch curGs = gs.copy();
                        Graph g = curGs.search(dataSubSamp);
