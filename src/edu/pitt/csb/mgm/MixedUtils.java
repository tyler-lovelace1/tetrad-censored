...
//        return new BoxDataSet(box, mixVars);
    ...
    //Takes a mixed dataset and returns only data corresponding to CensoredVariables in order
    public static DataSet getCensoredData(DataSet ds){
        ArrayList<Node> censVars = new ArrayList<Node>();
        for(Node n: ds.getVariables()){
            if(n instanceof CensoredVariable)
                censVars.add(n);
        }
        return ds.subsetColumns(censVars);
    }
...
//                        newTemp = templateDisc0;
    ...
    public static DoubleMatrix2D skeletonToMatrix(Graph graph, DataSet d) {
        ...
        HashMap<Node, Integer> map = new HashMap<Node, Integer>();
        for (Node node : graph.getNodes()) {
//        	if (node.getName().toLowerCase().contains("censor")) {
//        		continue;
//        	}
