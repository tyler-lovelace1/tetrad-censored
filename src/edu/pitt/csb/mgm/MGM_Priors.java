 ...
 */
public class MGM_Priors extends ConvexProximal implements GraphSearch{
    ...
    public MGM_Priors(DataSet ds, double[] lambda, double [] lambdaWP ,boolean [][] havePriors){
        this.havePriors = havePriors;
        ...
        this.p = xDat.columns();
        this.q = yDat.columns();
        contInd = new int[this.p];
        discInd = new int[this.q];

        int contCount = 0;
        int discCount = 0;
        for(int i = 0; i < ds.getNumColumns();i++)
        {
            if(ds.getVariable(i) instanceof ContinuousVariable)
            {
                contInd[contCount] = i;
                contCount++;
            }
      ...
      //  System.out.println(Arrays.toString(contInd));
     //   System.out.println(Arrays.toString(discInd));
        this.n = xDat.rows();
        ...
        this.lambdaWP = factory1D.make(lambdaWP);
    ...
    public DoubleMatrix1D proximalOperator(double t, DoubleMatrix1D X) {
        ...
        DoubleMatrix1D tlam = lambda.copy().assign(Functions.mult(t));
        DoubleMatrix1D tlamWP = lambdaWP.copy().assign(Functions.mult(t));
       ...
       // betascale.assign(Functions.max(0));
        ...
        DoubleMatrix2D weightMat = alg.multOuter(weights,
                weights, null);
        DoubleMatrix2D betaWeight = weightMat.viewPart(0, 0, p, p);
        DoubleMatrix2D betascale = betaWeight.copy();
        for(int i = 0; i < p; i ++)
        {
            for(int j = 0; j < p; j ++)
            {
                if(havePriors[contInd[i]][contInd[j]])
                    betascale.set(i,j,-tlamWP.get(0));
                else
                    betascale.set(i,j,-tlam.get(0));
            }
        }
        //  DoubleMatrix2D betascale = betaWeight.copy().assign(Functions.mult(-tlam.get(0))); //BS = -tlam.get(0)
        DoubleMatrix2D absBeta = par.beta.copy().assign(Functions.abs); //AB = |beta|
        betascale.assign(absBeta, Functions.div); // BS[i,j] = AB[i,j]/BS[i,j]
        betascale.assign(Functions.plus(1)); //BS[i,j] = BS[i,j] + 1
        betascale.assign(Functions.max(0)); //BS[i,j] = max(0,BS[i,j])
        //beta=beta.*betascale;
        ...
        */
        ...
        for(int i = 0; i < p; i++){
            for(int j = 0; j < lcumsum.length-1; j++){
                ...
                double thetaScale;
                if(havePriors[contInd[i]][discInd[j]])
                    thetaScale = Math.max(0, 1 - tlamWP.get(1) * weightMat.get(i, p+j)/norm2(tempVec));
                else
                    thetaScale = Math.max(0, 1 - tlam.get(1) * weightMat.get(i, p+j)/norm2(tempVec));
        ...
        for(int i = 0; i < lcumsum.length-1; i++){
            for(int j = i+1; j < lcumsum.length-1; j++){
                ...
                double phiScale;
                if(havePriors[discInd[i]][discInd[j]])
                    phiScale = Math.max(0, 1 - tlamWP.get(2) * weightMat.get(p + i,p+j)/norm2(tempMat));
                else
                    phiScale = Math.max(0, 1 - tlam.get(2) * weightMat.get(p + i,p+j)/norm2(tempMat));
    ...
    public double nonSmooth(double t, DoubleMatrix1D X, DoubleMatrix1D pX) {
      ...
      //  DoubleMatrix2D betascale = betaWeight.copy().assign(Functions.mult(-tlam.get(0))); //BS = -tlam.get(0)
        ...
        for(int i= 0; i < p; i++){
            for(int j = 0; j < p; j++){
                ...
                if(curVal !=0){
                    ...
                    if (havePriors[contInd[i]][contInd[j]])
                        betaNormsWP += Math.abs(betaWeight.get(i,j)*curVal);
                    else
        ...
        */
        ...
        for(int i = 0; i < p; i++){
            for(int j = 0; j < lcumsum.length-1; j++){
                ...
                if(havePriors[contInd[i]][discInd[j]])
                {
                    ...
                    //double thetaScale = Math.max(0, 1 - tlam.get(1)*weightMat.get(i, p+j)/Math.sqrt(alg.norm2(tempVec)));
                    double foo = norm2(tempVec);
                    double thetaScale = Math.max(0, 1 - tlamWP.get(1) * weightMat.get(i, p+j)/norm2(tempVec));
                    tempVec.assign(Functions.mult(thetaScale));
                    thetaNormsWP += weightMat.get(i, p+j)*Math.sqrt(alg.norm2(tempVec));
                }
                else {
                    DoubleMatrix1D tempVec = par.theta.viewColumn(i).viewPart(lcumsum[j], l[j]);
        ...
        for(int i = 0; i < lcumsum.length-1; i++){
            for(int j = i+1; j < lcumsum.length-1; j++){
                ...
                if(havePriors[discInd[i]][discInd[j]])
                {
                    ...
                    //not sure why this isnt Frobenius norm...
                    //double phiScale = Math.max(0, 1-tlam.get(2)*weightMat.get(p+i,p+j)/alg.norm2(tempMat));
                    double phiScale = Math.max(0, 1 - tlamWP.get(2) * weightMat.get(p + i,p+j)/norm2(tempMat));
                    //double phiScale = Math.max(0, 1-tlam.get(2)*weightMat.get(p+i,p+j)/alg.normF(tempMat));
                    tempMat.assign(Functions.mult(phiScale));
                    phiNormsWP += weightMat.get(p+i,p+j)*alg.normF(tempMat);
                }
                else {
                    DoubleMatrix2D tempMat = par.phi.viewPart(lcumsum[i], lcumsum[j], l[i], l[j]);
        ...
        return lambda.get(0)*betaNorms + lambdaWP.get(0)*betaNormsWP + lambda.get(1)*thetaNorms + lambdaWP.get(1)*thetaNormsWP + lambda.get(2)*phiNorms + lambdaWP.get(2)*phiNormsWP;
    }
