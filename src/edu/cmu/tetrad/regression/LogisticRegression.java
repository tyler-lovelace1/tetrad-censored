...
public class LogisticRegression implements TetradSerializable {
     ...
     */
    public Result regress(DiscreteVariable x, List<Node> regressors, int[] _rows) {
        if (!binary(x)) {
            throw new IllegalArgumentException("Target must be binary.");
        }

        for (Node node : regressors) {
            if (!(node instanceof ContinuousVariable || binary(node) || node instanceof CensoredVariable)) {
                throw new IllegalArgumentException("Regressors must be continuous or binary.");
            }
        }

        double[][] _regressors = new double[regressors.size()][_rows.length];
        ...
        for (int j = 0; j < regressors.size(); j++) {
            ...
            for (int i = 0; i < _rows.length; i++) {
                _regressors[j][i] = dataCol[_rows[i]];
            }
        }

        int[] target = new int[_rows.length];
        int col = dataSet.getColumn(dataSet.getVariable(x.getName()));

        for (int i = 0; i < _rows.length; i++) {
            target[i] = dataSet.getInt(_rows[i], col);
        }
    ...
    public Result regress(DiscreteVariable x, List<Node> regressors, int[] _rows, List<Node> cens, double[][] expVals) {
        if (!binary(x)) {
            throw new IllegalArgumentException("Target must be binary.");
        }
        ...
        for (Node _n1 : cens) {
            ...
            for (Node _n2 : regressors) {
                if (_n1.getName() == _n2.getName()) {
                    for (int i = 0; i < _rows.length; i++) {
                        _regressors[idx][i] = expVals[expidx][i];
                    }
                }
                idx++;
            }
