...
//    private NumberFormat nf = NumberFormatUtil.getInstance().getNumberFormat();
    ...
    public RegressionResult regress(Node target, List<Node> regressors, List<Node> cens, DoubleMatrix2D expVals) {
        int n = getRows().length;
        int k = regressors.size() + 1;
        ...
        for (Node _n1 : cens) {
            ...
            for (Node _n2 : regressors) {
                if (_n1.getName() == _n2.getName()) {
                    for (int i = 0; i < xSub.rows(); i++) xSub.set(i, idx, expVals.get(i,col));
                }
                idx++;
            }
            col++;
