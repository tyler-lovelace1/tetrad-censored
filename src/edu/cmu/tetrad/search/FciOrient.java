...
public final class FciOrient {
     ...
     */
    private Map<String,String>whyOrient;
    ...
    public FciOrient(SepsetProducer sepsets) {
        ...
        this.whyOrient = new ConcurrentHashMap<String,String>();
    ...
    public FciOrient(SepsetProducer sepsets,Map<String,String> whyOrient) {
        this.sepsets = sepsets;
    ...
    private void ruleR2(Node a, Node b, Node c, Graph graph) {
        if ((graph.isAdjacentTo(a, c))
            ...
            if ((graph.getEndpoint(a, b) == Endpoint.ARROW)
                ...
                whyOrient.put(a.getName() + "," + c.getName(),"2," + b.getName());
        ...
        for (Node B : nodes) {
            ...
            for (Node D : intoBCircles) {
                ...
                while ((choice = gen.next()) != null) {
                    ...
                    graph.setEndpoint(D, B, Endpoint.ARROW);
                    whyOrient.put(D.getName() + "," + B.getName(),"3,"+A.getName()+","+C.getName());
    ...
    private boolean doDdpOrientation(Node d, Node a, Node b, Node c, Map<Node, Node> previous, Graph graph) {
        if (dag != null) {
            if (dag.isAncestorOf(b, c)) {
                graph.setEndpoint(c, b, Endpoint.TAIL);
                whyOrient.put(c.getName() + "," + b.getName(),"4");
                changeFlag = true;
            } else {
                if (!isArrowpointAllowed(a, b, graph)) {
                    return false;
                ...
                graph.setEndpoint(a, b, Endpoint.ARROW);
                graph.setEndpoint(c, b, Endpoint.ARROW);
                whyOrient.put(a.getName() + "," + b.getName(),"4");
                whyOrient.put(c.getName() + "," + b.getName(),"4");
                changeFlag = true;
            }
