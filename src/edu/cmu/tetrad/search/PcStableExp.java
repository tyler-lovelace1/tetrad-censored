 ...
 */
public class PcStableExp implements GraphSearch {
    ...
    private boolean contFirst = false;
    /**
     ...
     */
    public PcStableExp(IndependenceTest independenceTest,boolean contFirst) {
    ...
    /*When doing a partitioned mixed search (search over continuous variables first), this method adds
    the discrete nodes back into the learned continuous graph
     */
    private Graph addDiscreteNodes(Graph g, DataSet d)
    {
...
//        this.logger.log("info", "Variables " + independenceTest.getVariables());
        ...
        if(contFirst)
        {
            ...
            DataSet all = (DataSet)getIndependenceTest().getData();
            DataSet cont = MixedUtils.getContinousData(all);
            graph = new EdgeListGraph(cont.getVariables());
            IndependenceTest temp = new IndTestFisherZ(cont,getIndependenceTest().getAlpha());
           // initialGraph = graph;
            IFas fas = new FasStableConcurrent(initialGraph, temp);
            fas.setKnowledge(getKnowledge());
            fas.setDepth(getDepth());
            fas.setVerbose(verbose);
            graph = fas.search();
        //    System.out.println("Continuous Graph:" + graph);
            sepsets = fas.getSepsets();
            this.graph = addDiscreteNodes(graph,all);
          //  System.out.println("With Discrete Variables: " + graph);
            fas = new FasStableConcurrent(graph,getIndependenceTest());
            fas.setKnowledge(getKnowledge());
            fas.setDepth(getDepth());
            fas.setVerbose(verbose);
            graph = fas.search();
        //    System.out.println("Final Adjacencies: " + graph);
            sepsets.addAll(fas.getSepsets());
