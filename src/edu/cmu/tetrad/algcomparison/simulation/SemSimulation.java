...
public class SemSimulation implements Simulation {
    ...
    public void createData(Parameters parameters) throws Exception {
        ...
        for (int i = 0; i < parameters.getInt("numRuns"); i++) {
            ...
            DataSet dataSet = simulate(graph, parameters,i+1);
...
//        }
    ...
    private DataSet simulate(Graph graph, Parameters parameters,int run) throws Exception {
        File dir = new File("ImplCovarMatrices");
        dir.mkdirs();
        File implCovar = new File(dir,"ImplCov_VS"+parameters.getInt("numMeasures")+"_Run"+run+"_C.txt");
        PrintStream icOut = new PrintStream(implCovar);
        SemIm im = this.im;
        ...
        if (im == null) {
            ...
            if (pm == null) {
                ...
                TetradMatrix t  = im.getImplCovar(false);
                //save t to file
                icOut.println(t.toString());
                ims.add(im);
