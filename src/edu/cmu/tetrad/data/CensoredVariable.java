 ...
 */
public final class CensoredVariable extends AbstractVariable
    ...
    /**
     *  Name of binary event variable
     */
    private String censorName;

    /**
     *  Binary censoring variable
     */
    private int[] censor;

    /**
     *  ranking of samples by survival time
     */
    private int[] order;
    ...
    /**
     *  indices of complete (uncensored) cases
     */
    private int[] CC;

    /**
     *  expected value of censored values for use in QL independence test
     */
     ...
     */
    public CensoredVariable(String name) {
        super(name);
    }
     ...
     * @param name the name of the variable.
     */
    public CensoredVariable(String name, String censorName) {
        super(name);
     ...
     * Copy constructor.
     */
    public CensoredVariable(CensoredVariable variable) {
        super(variable.getName());
        this.nodeType = variable.nodeType;
        this.centerX = variable.centerX;
        this.centerY = variable.centerY;
        this.setCensor(variable.getCensor());
    }

    /**
     * Generates a simple exemplar of this class to test serialization.
     */
    public static CensoredVariable serializableInstance() {
        return new CensoredVariable("X");
    }
    ...
    public Node like(String name) {
        CensoredVariable censoredVariable = new CensoredVariable(name);
        censoredVariable.setNodeType(getNodeType());
        return censoredVariable;
    }

    /**
     * set the name of binary censor variable
     */
    ...
    /**
     * get the name of binary censor variable
     */
    public String getCensorName() { return this.censorName; }

    /**
     *  set the array of binary censor values
     */
    public void setCensor(int[] censor) {
        this.n = censor.length;
        this.censor = new int[this.n];
        int numCC = 0;
        for (int i = 0; i < this.n; i++) {
            this.censor[i] = censor[i];
            numCC += censor[i];
        }
        int idx = 0;
        this.CC = new int[numCC];
        for (int i = 0; i < this.n; i++) {
            if (censor[i] == 1) {
                this.CC[idx] = i;
                idx++;
    ...
    /**
     *  get the array of binary censoring values
     */
    ...
    /**
     *  get the binary censoring value at index i
     */
    public int getCensor(int i) {
        if (i < 0 || i >= this.n) {
            throw new IllegalArgumentException("index for " + i + " censored variable "
                    + getName() + " is out of bounds");
        }
        return this.censor[i];
    }

    public void setExpval(double[] expval) {
        if (this.n != expval.length) throw new IllegalArgumentException("length og expected value array must equal length of censoring array");
        this.expval = new double[this.n];
        for (int i = 0; i < this.n; i++) this.expval[i] = expval[i];
    }
    ...
    public double getExpval(int i) {
        return this.expval[i];
    }

    /**
     *  set the survival time order and number of ties for each survival time
     */
    public void setOrder(double[] times) {
        if (times.length != this.n) {
            throw new IllegalArgumentException("length of times array, " + times.length
                    + " does not equal the number of samples, " + this.n);
        }
        this.order = new int[this.n];

        List<Double> _times = new ArrayList<>();
        for (double t : times) {
            if (t < 0) {
                throw new IllegalArgumentException("Censored variable times cannot be negative");
            }
            _times.add(t);
        }
        List<Double> sorted = new ArrayList<>(_times);
        Collections.sort(sorted);

        int idx = 0;
        while (idx < this.n) {
            for (int i = 0; i < this.n; i++) {
                if (sorted.get(idx) == _times.get(i)) {
                    order[idx] = i;
                    _times.set(i, -1.0);
                    idx++;
                    if (idx == this.n) {
...
//        System.out.println("unique times for " + getName() + ": " + this.unique);

        this.H = new int[this.unique];
    ...
    public int getH(int i) {
        if (i < 0 || i >= this.unique) {
            throw new IllegalArgumentException("index for unique time " + i + " censored variable "
                    + getName() + " is out of bounds");
        }
        return this.H[i];
    }
    ...
    /**
     *
     * @return an array of indices of complete (uncensored) cases
     */
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof CensoredVariable)) {
            return false;
        }
        if (NodeEqualityMode.getEqualityType() == NodeEqualityMode.Type.OBJECT) {
            return o == this;
