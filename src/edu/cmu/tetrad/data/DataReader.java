...
//            }
        ...
        List<Node> survList = new ArrayList<>();
        List<Node> cenList = new ArrayList<>();
        for (Node n : variables) {
            if (n instanceof CensoredVariable) {
                survList.add(n);
                cenList.add(dataSet.getVariable(((CensoredVariable) n).getCensorName()));
            }
        }

        for (int j = 0; j < cenList.size(); j++) {
            int[] censor = new int[description.getNumRows()];
            double[] time = new double[description.getNumRows()];
            for (int i = 0; i < censor.length; i++) {
                censor[i] = dataSet.getInt(i,dataSet.getColumn(cenList.get(j)));
                time[i] = dataSet.getDouble(i, dataSet.getColumn(survList.get(j)));
            }
            ((CensoredVariable) survList.get(j)).setCensor(censor);
            ((CensoredVariable) survList.get(j)).setOrder(time);
        }

        for (Node n : cenList) {
            dataSet.removeColumn(n);
        }
...
//        LogUtils.getInstance().info("ID label = " + idLabel);
    ...
    private void setValue(DataSet dataSet, int row, int col, String s) {
        ...
        } else if (node instanceof CensoredVariable) {
            try {
                double value = Double.parseDouble(s);
                dataSet.setDouble(row, col, value);
            } catch (NumberFormatException e) {
...
//        }
    ...
    private DataSetDescription scanForDescription(List<String> varNames,
        ...
        System.out.println("Max number of categories: " + maxIntegralDiscrete);
        ...
        List<String> survList = new ArrayList<>();
        List<String> cenList = new ArrayList<>();
        ...
        for (int i = 0; i < varNames.size(); i++) {
            ...
            if (varNames.get(i).toLowerCase().contains("survival")) {
                String name = varNames.get(i);
                survList.add(name);
                variables.add(new CensoredVariable(name));
            } else if (tooManyDiscreteValues(strings)) {
                String name = varNames.get(i);
...
//                        }
                ...
                if (name.toLowerCase().contains("censor")) {
                    if (categories.size() != 2)
                        throw new IllegalArgumentException("Censoring event variable must be binary");
                    cenList.add(name);
                }
...
//            }
        ...
        if (survList.size() != cenList.size()) {
            System.out.println(survList);
            System.out.println(cenList);
            throw new IllegalArgumentException("Number of censored time variables does not match number of censored event variables");
        } else {
            int i = 0;
            for (Node n : variables) {
                if (n instanceof CensoredVariable){
                    ((CensoredVariable) n).setCensorName(cenList.get(i));
                    i++;
                }
            }
        }
