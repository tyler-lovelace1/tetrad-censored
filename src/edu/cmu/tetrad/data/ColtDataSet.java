...
//                    "has already been used: " + id);
    ...
    public final boolean isMixed() {
        ...
        for (int i = 0; i < getNumColumns(); i++) {
            ...
            if (column instanceof ContinuousVariable) {
                ...
                numDiscrete++;
            } else if (!(column instanceof CensoredVariable)) {
                throw new IllegalArgumentException(
        ...
        return (numContinuous > 0 && numDiscrete > 0) || isCensored();
    }

    /**
     * @return true if this is a censored tetradMatrix set--that is, if it contains at
     * least one censored column.
     */
    public final boolean isCensored() {
        ...
        for (int i = 0; i < getNumColumns(); i++) {
            ...
            if (column instanceof CensoredVariable) {
                return true;
            } else if (!(column instanceof DiscreteVariable) && !(column instanceof ContinuousVariable)) {
                throw new IllegalArgumentException(
                        "Column not of type continuous" +
                                "or of type discrete; can't classify this tetradMatrix set.");
            }
    ...
    public final String toString() {
        ...
        for (int i = 0; i < getNumRows(); i++) {
            ...
            for (int j = 0; j < getNumColumns(); j++) {
                ...
                } else if (variable instanceof CensoredVariable) {
                    if (Double.isNaN(getDouble(i, j))) {
                        buf.append("*");
                    } else {
                        buf.append(getNumberFormat().format(getDouble(i, j)));
                        buf.append("_");
                        buf.append(((CensoredVariable) variable).getCensor(i));
                    }
...
//        }
    ...
    /**
     * @return a TetradMatrix containing the censoring data for censored variables
     * @throws IllegalStateException if this is not a censored dataset.
     */
    public final TetradMatrix getDoubleCensorData() {
        if (!isCensored()) {
            throw new IllegalStateException("DataSet must contain censored data");
        }
        List<CensoredVariable> censVars = new ArrayList<>();
        int numCensored = 0;
        for (Node n : variables){
            if (n instanceof CensoredVariable){
                numCensored++;
                censVars.add((CensoredVariable) n);
            }
        }

        int n = tetradMatrix.rows();

        TetradMatrix censVals = new TetradMatrix(n, numCensored);

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < numCensored; j++) {
                censVals.set(i, j, (double) censVars.get(j).getCensor(i));
            }
        }
...
//        _dataSet.multipliers = new HashMap<>(multipliers);
    ...
    public final DataSet subsetRows(int rows[]) {
        ...
        for (int j = 0; j < _data.variables.size(); j++) {
            if (_data.variables.get(j) instanceof CensoredVariable) {
                CensoredVariable _n = new CensoredVariable((CensoredVariable) _data.variables.get(j));
                int[] censor = _n.getCensor();
                int[] _censor = new int[rows.length];
                for (int i = 0; i < rows.length; i++) {
                    _censor[i] = censor[rows[i]];
                }
                _n.setCensor(_censor);
                _n.setOrder(_data.tetradMatrix.getColumn(j).toArray());
                _data.variables.remove(_data.variables.get(j));
                _data.variables.add(j, _n);
            }
        }
